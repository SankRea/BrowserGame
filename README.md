...
- 11 个手工关卡：8 关入门 + 3 张重新设计的挑战地图；用有限的墙，保留指定蓝线并排除所有其他起终点路线。
- 三张挑战地图的目标路线分别为直线、L 形和 U 形；难点来自支路的汇合与切断位置，所有白色空地均可放墙。
- 已完整求解确认最少墙数分别为 3、4、5；模拟方法和局限见 [关卡分析](docs/only-path-analysis.md)。
...
- 页面可直接进入挑战，关卡选择分为入门与挑战；小屏幕可放大棋盘，在棋盘内上下左右滑动。
- 第一挑战关直达：`/pages/only-path.html?level=challenge-quiet-river`，也可替换为其他关卡 ID。
...
日常修改保留在本地，仅在用户明确要求上传时推送到 GitHub；发布也按明确要求执行。
...
「唯一的路」已进行 Node 关卡模拟与自动化测试，尚未进行构建或浏览器交互验证。GitHub Pages 使用上面的手动发布流程，上传源码后需重新运行工作流。
